  ...
  <div id="wordTool">
    <a-row type="flex" class="start">
      <a-col class="left" flex="auto">
        <!-- 说明 -->
        <a-typography-title :level="2">文言实词解析工具</a-typography-title>
        <a-typography-title :level="4">说明</a-typography-title>
        <p>支持统编版高中语文书电子版课下注释解析</p>
        <p>可前往<a href="https://basic.smartedu.cn/tchMaterial?defaultTag=e7bbcefe-0590-11ed-9c79-92fc3b3249d5%2F6a749654-0772-11ed-ac74-092ab92074e6%2F44bee8bc-54e6-11ed-9c34-850ba61fa9f4" target="_blank">智慧中小学网站</a>复制文言文原文和注释</p>
        <!-- 文本域 -->
        <a-textarea
          v-model:value="wordListString"
          placeholder="请文言文原文和注释"
          :auto-size="{ minRows: 5, maxRows: 5 }"
          style="width: 100%;"
        />
        <a-button type="primary" @click="parseWordList">解析</a-button>
        <a-button @click="addCard">添加</a-button>
...
import * as wywTool from '../../utils/wywTool';

const wordListString = ref(''); // 文言文字符串
const dataSource = ref<any>([]); // 文言文

const parseWordList = () => {
  if (!wordListString.value) {
    message.error('请输入文言文');
    return;
  }
  debugger;
  // 解析文言文
  const wyList = wywTool.parseComment(wordListString.value)
  // 转换格式
  const data = wywTool.toQues(wyList)
  if (!data.length) {
    message.error('解析失败');
    return;
  }
...
const addTempData = () => {
  ...
  Modal.confirm({
    ...
    onOk() {
      ...
      // 添加至临时数据
      addTempQuestion(tempDataName.value, dataSource.value)
...
const columns = [
  ...
  { title: '虚词', dataIndex: 'q' },
  { title: '注释', dataIndex: 'ans' },
